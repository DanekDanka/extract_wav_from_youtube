...
class AudioDatasetCreator:
    ...
    def process_audio(self):
        ...
        # Проверяем, что у каждого ID ровно 5 записей
        if len(self.entries) != 5:
            messagebox.showerror("Ошибка", f"Должно быть ровно 5 записей, найдено {len(self.entries)}")
            return
        ...
        for entry in self.entries:
            ...
            for j, time_entry in enumerate(time_entries):
                ...
                if self.extract_audio_segment(str(temp_file.with_suffix('.wav')), str(segment_file), start_time, end_time):
            ...
            # Удаляем временный файл
            try:
                if temp_file.with_suffix('.wav').exists():
                    temp_file.with_suffix('.wav').unlink()
            except:
                pass
